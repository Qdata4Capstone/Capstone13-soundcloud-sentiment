$(document).ready(function(){
    ...
    $('input.search-page').on('typeahead:selected', function(obj, datum, name) {
        var currentSeries = $.map($('#chart').highcharts().series, function(n, i){
            return n['name'];
        });

        console.log(currentSeries);

        if(currentSeries.indexOf(datum.value) === -1){
            $('#chart').highcharts().addSeries({
                 name: datum.value,
                 data: meta[datum.value]['data']
             });
        }
    });
